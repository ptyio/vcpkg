function(configure_qt)
    ...
    foreach(_buildname ${BUILDTYPES})
        ...
        if(VCPKG_TARGET_IS_LINUX AND (VCPKG_TARGET_ARCHITECTURE STREQUAL "arm" OR VCPKG_TARGET_ARCHITECTURE STREQUAL "arm64"))
            message(STATUS "arm linux detected")
            list(REMOVE_ITEM BUILD_OPTIONS "-qt-xcb")
            list(REMOVE_ITEM BUILD_OPTIONS "-xcb")
            list(REMOVE_ITEM BUILD_OPTIONS "-xcb-xlib")
            list(APPEND BUILD_OPTIONS -no-opengl -sysroot ${VCPKG_ARM_LINUX_SYSROOT} -extprefix ${CURRENT_INSTALLED_DIR} -skip qtwayland -skip qtlocation -skip qtscript)

            if(VCPKG_TARGET_ARCHITECTURE STREQUAL "arm")
                list(APPEND BUILD_OPTIONS -device linux-rasp-pi-g++ -device-option CROSS_COMPILE=${VCPKG_ARM_LINUX_TOOLCHAIN}/bin/arm-linux-gnueabihf-)
            endif()
            if(VCPKG_TARGET_ARCHITECTURE STREQUAL "arm64")
                list(APPEND BUILD_OPTIONS -xplatform linux-aarch64-gnu-g++)
            endif()
        endif()

        if(VCPKG_TARGET_IS_OSX AND VCPKG_TARGET_ARCHITECTURE STREQUAL "arm64")
            message(STATUS "arm osx detected")
            file(COPY ${_csc_SOURCE_PATH}/mkspecs/macx-clang DESTINATION ${_csc_SOURCE_PATH}/mkspecs/tmp)
            file(RENAME ${_csc_SOURCE_PATH}/mkspecs/tmp/macx-clang ${_csc_SOURCE_PATH}/mkspecs/tmp/macx-clang-arm64)
            file(COPY ${_csc_SOURCE_PATH}/mkspecs/tmp/macx-clang-arm64 DESTINATION ${_csc_SOURCE_PATH}/mkspecs)
            file(REMOVE ${_csc_SOURCE_PATH}/mkspecs/tmp)
            file(READ "${_csc_SOURCE_PATH}/mkspecs/macx-clang-arm64/qmake.conf" _tmp_contents)
            string(REPLACE "load(qt_config)" "QMAKE_APPLE_DEVICE_ARCHS = arm64\n\nload(qt_config)" _tmp_contents ${_tmp_contents})
            file(WRITE "${_csc_SOURCE_PATH}/mkspecs/macx-clang-arm64/qmake.conf" ${_tmp_contents})

            list(REMOVE_ITEM BUILD_OPTIONS "-platform ${_csc_TARGET_PLATFORM}")
            #list(APPEND BUILD_OPTIONS -platform macx-clang)
            list(APPEND BUILD_OPTIONS -xplatform macx-clang-arm64)
        endif()

        if(VCPKG_TARGET_IS_IOS)
            message(STATUS "iOS build detected (can be built only on intel x64 macOS host, not on arm64 host for Qt 5.15, just not supported host)")
            message(STATUS "iOS todo find a way to build all vcpkg libs with bitcode enabled")

            list(REMOVE_ITEM BUILD_OPTIONS "-platform ${_csc_TARGET_PLATFORM}")
            list(REMOVE_ITEM BUILD_OPTIONS "-platform macx-ios-clang")
            list(REMOVE_ITEM BUILD_OPTIONS "-platform")
            list(REMOVE_ITEM BUILD_OPTIONS "macx-ios-clang")
            #string(REPLACE "-platform macx-ios-clang" "" BUILD_OPTIONS ${BUILD_OPTIONS})
            #string(REPLACE "-platform ${_csc_TARGET_PLATFORM}" "" BUILD_OPTIONS ${BUILD_OPTIONS})
            #message(STATUS "BUILD_OPTIONS ${BUILD_OPTIONS}")

            # todo find a way how to build all vcpkg libs with bitcode
            file(READ "${_csc_SOURCE_PATH}/mkspecs/common/uikit.conf" _tmp_contents)
            string(REPLACE "bitcode" "   " _tmp_contents ${_tmp_contents})
            file(WRITE "${_csc_SOURCE_PATH}/mkspecs/common/uikit.conf" ${_tmp_contents})

            list(APPEND BUILD_OPTIONS -xplatform macx-ios-clang)
        endif()

        # NOTE: required to be installed `x64-windows-static` or another host triplet Qt version to use host qmake (https://vcpkg.readthedocs.io/en/latest/users/host-dependencies/)
        if(VCPKG_TARGET_IS_WINDOWS AND VCPKG_TARGET_ARCHITECTURE STREQUAL "arm64")
            message(STATUS "arm windows detected")
            list(REMOVE_ITEM BUILD_OPTIONS "-platform ${_csc_TARGET_PLATFORM}")
            list(APPEND BUILD_OPTIONS -platform win32-msvc -xplatform win32-arm64-msvc2017 -external-hostbindir ${CURRENT_HOST_INSTALLED_DIR}/tools/qt5-base/bin)
        endif()
        ...
        if(VCPKG_TARGET_IS_WINDOWS AND VCPKG_TARGET_ARCHITECTURE STREQUAL "arm64")
            message(STATUS "WinArm64Replace HostPrefix=${CURRENT_INSTALLED_DIR} -> HostPrefix=${CURRENT_HOST_INSTALLED_DIR}")
            message(STATUS "HostPrefixBefore ${_contents}")
            string(REPLACE "HostPrefix=\${CURRENT_INSTALLED_DIR}" "HostPrefix=\${CURRENT_HOST_INSTALLED_DIR}" _contents ${_contents})
            message(STATUS "HostPrefixAfter ${_contents}")
        endif()
